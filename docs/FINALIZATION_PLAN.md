...
- AI Usage Statement wording: per Dr. Joshua's explicit instruction (12 Jul 2026), keep this section minimal. State only that Claude Code was used for code implementation under student supervision, and Perplexity was used to find research articles (verified against source PDFs before citing). Do not add anything beyond this. See "AI usage disclosure" guardrail below.
- BLOCKER: no Azure OpenAI credentials (.env missing). Blocks dense embeddings, agent runs, H1/H2 testing, O3/O4/O5 completion. UPDATE 12 Jul (post Meeting 3): Polytechnic will not provide an institutional Azure OpenAI subscription. Dr. Joshua relayed a suggestion from Dr. Philippe (research office) to apply for the AWS Cloud Credit for Research programme (https://aws.amazon.com/government-education/research-and-technical-computing/cloud-credit-for-research/). If approved, decide whether to use Amazon Bedrock (architecture change, needs updating in Final Thesis away from "Azure OpenAI") or fund an OpenAI-compatible endpoint with the credits (keeps existing architecture as proposed). Fallback if AWS credits are delayed/denied: personal OpenAI subscription, likely low cost for this project's actual usage.
- After the Mid Review Report is sent to Dr. Joshua for review, expect a follow-up request to present/walk through the work done so far. This is separate from the formal Template D demonstration (due 23 Aug). Prepare a short informal walkthrough (progress, live dashboard demo, bug-fix story) when this is requested.
...
5. Writing and revision workflow, revised 12 Jul 2026: automated stylistic revision passes on reference-dense text reliably corrupt IEEE numeric citation markers (dropped brackets, misattributed sources, even with placeholder-token substitution). Fix: draft citation-dense sections using name-year form (e.g. "Lewis et al. (2020)") during any revision pass, since names and years survive revision intact, then convert back to IEEE numeric `[n]` afterward by matching each name-year mention to its position in the References list. Always re-verify every citation and every fact after any automated revision pass; documented failures include misspelling "Claude Code," rewriting solo work as "the team," and lowercasing proper nouns and acronyms (azure openai, chromadb, bm25). Manually fix every such error before use. Do not reference any specific third-party revision tool by name in submitted documents.
